...
Используйте семантические переменные вместо hardcoded значений:

```css
@theme {
  /* Отступы */
  --spacing-section: 3em;    /* Большие разрывы (hr, секции) */
  --spacing-element: 1.25em; /* Между параграфами, списками */
  --spacing-small: 0.5em;    /* Элементы списков, мелкие отступы */
  
  /* Типографика */
  --line-height-heading: 1.25; /* Заголовки */
  --line-height-body: 1.75;    /* Основной текст */
  
  /* Скругления */
  --radius-sm: 0.25rem; /* Мелкие элементы (код, бейджи) */
  --radius: 0.5rem;     /* Стандарт (карточки, кнопки) */
}
```

**Преимущества:**
- Единый источник истины для значений
- Легко менять отступы согласованно по всему приложению
- Лучшая поддерживаемость
- Ясное семантическое значение
...
5. **Семантика** - переменные передают намерение, а не просто значения
...
### Мгновенное переключение без transitions

Для устранения промежуточных состояний при переключении между светлой и темной темой используется **мгновенное переключение без CSS transitions**.

**Почему БЕЗ transitions?**

В приложении используются два механизма стилизации:
1. **CSS переменные** (`var(--color-background)`, `var(--color-foreground)`)
2. **Tailwind inline классы** (`dark:prose-invert`, `dark:bg-gray-800`)

Tailwind применяет `dark:` классы **мгновенно** при добавлении класса `.dark` к `<html>`. Если добавить CSS transitions только для переменных, возникает рассинхронизация:
- Tailwind классы переключаются мгновенно
- CSS переменные переключаются с задержкой (0.05-0.15s)
- Результат: видимые промежуточные состояния

**Решение: Мгновенное переключение**

```css
* {
  border-color: var(--color-border);
  /* НЕТ transitions - синхронно с Tailwind классами */
}
```

**Конфигурация:**

1. **Color-scheme meta** для browser-native оптимизации:
```css
:root {
  color-scheme: light;
}
...
3. **Hydration warnings suppression**:
```tsx
...
**Результат:**
- Мгновенное синхронное переключение темы
- Нет промежуточных состояний между CSS переменными и Tailwind классами
- Работает одинаково для переключения на светлую и темную тему
- Нет flickering при загрузке страницы
- Browser-native оптимизация через `color-scheme`

**Важно:** Мгновенное переключение лучше, чем transitions с рассинхронизацией. Для плавности можно добавить transitions только на **специфичные** элементы, где нет конфликта с Tailwind классами.
