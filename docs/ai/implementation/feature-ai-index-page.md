...
- Add a summary-level `Index loaded notes` action that only processes the currently loaded rows after active filter + active committed search have already narrowed the dataset.
...
- Treat the bulk summary action as an orchestrator over the existing row-mutation path rather than a separate backend workflow. It should derive its candidate set from the already loaded list, skip `indexed` rows, and feed each success back through the same row-update pipeline.
...
- The mobile bulk action should live in the same summary area as the count/reset affordances so it mirrors the web placement and inherits the same "loaded notes only" mental model.
