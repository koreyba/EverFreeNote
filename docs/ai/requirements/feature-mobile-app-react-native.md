...
- **React Native версия:** 0.77.3 (последняя стабильная с новой архитектурой)
- **Expo SDK:** 54 (поддержка всех нужных нативных модулей)
- **Supabase JS SDK:** v2.x (та же версия, что и в web)
- **TipTap:** Используется через react-native-webview - 100% переиспользование существующего RichTextEditor компонента
- **TanStack Query:** Полная совместимость с React Native
- **expo-sqlite:** Для локального хранения заметок (AsyncStorage ограничен 6MB)
