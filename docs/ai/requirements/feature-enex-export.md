...
- Primary goals
  ...
  - Совместимость ресурсы/ENML с Evernote (hash/en-media + resources) для корректной визуализации изображений
...
7. Изображения скачиваются из Supabase Storage и конвертируются в base64, считаются md5-хэши и формируются ресурсы для <en-media> (проблемные изображения пропускаются)
...
- Measurable outcomes
  ...
  - ✅ Связь <en-media> c ресурсами через md5-хэш как в Evernote
  - ✅ Пользователь может выбрать заметки через UI с карточками и чекбоксами
  - ✅ Кнопка "Выбрать все" работает корректно

- Acceptance criteria
  - Экспорт/импорт идемпотентен: импортированный .enex при повторном экспорте совпадает по содержимому (кроме export-date)
...
- Assumptions we're making
  ...
  - Ресурсы содержат md5-хэши и используются в <en-media> по правилам Evernote
