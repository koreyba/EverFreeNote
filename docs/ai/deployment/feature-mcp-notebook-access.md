...
| Production | `pmlloiywmuglbjkhrggo` ("EverFreeNote", dkoreiba account) | Fully live 2026-09-10: OAuth server on, authorization path `/oauth/consent`, dynamic registration on, Site URL `https://everfreenote.pages.dev`, function deployed, JWT keys rotated to ECC P-256. | OAuth server enabled, authorization path `/oauth/consent`, dynamic client registration ON (done 2026-09-10). Site URL `https://everfreenote.pages.dev` is already concrete; consent page goes live when this branch reaches the production deploy. Function deployed 2026-09-10. |
...
**Restart the project immediately after rotating.** On stage the rotation was left to settle and Auth plus the Data API stayed dead for about 20 minutes; on production the restart was issued straight away and the gap was roughly one minute. Observed on stage, 2026-09-10: After the rotation, Auth and the Data API (PostgREST) stopped responding entirely (connection failures, not 5xx) and the site was unusable for about 20 minutes. Edge Functions and Storage were unaffected. **Restarting the project** (Settings → General → Restart project) brought everything back, and `/auth/v1/.well-known/jwks.json` then served the ES256 key. Treat the rotation as a maintenance window and restart the project immediately afterwards rather than waiting.
