...
## Remote deletion after actual creation (2026-09-22 follow-up)
- Reproduction: two Cypress cases create notes through the real editor/save/queue path, one online and one with failed transport followed by reconnect. Both confirm a synced cache and empty queue, return to the list with Back, then simulate remote deletion at the HTTP boundary. On d16f42c both failed because the card remained; after the guard correction all five OfflineNotes component cases pass (zero retries). These are browser component tests, not a remote-provider E2E.
- Controller tests exercise saved, dirty, saving and error hidden tabs. Reverting only the guard fails the saved-tab case; restoring it passes 53 targeted controller/refresh tests. Existing refresh tests retain abort/account/edit/queue/revision protection. Reports: `.tmp-artifacts/redelete-red`, `redelete-green`, `redelete-guard-red`, `redelete-final-unit`.
- Android emulator (API 36, installed dev shell, real local Supabase): an actual online-created note reproduces the stale card on the old APK. On the fixed APK, both an online-created note and an offline-created-then-synced note disappear from cards, IndexedDB and workspace tabs after one ADB touch swipe following SQL deletion scoped to the test account. Offline transport was verified unavailable after removing adb reverse and cold-launching; navigator.onLine remained true. Queue was empty and cache synced before each remote deletion. Evidence: `.tmp-artifacts/redelete-native.jsonl`. No physical phone execution is claimed.
- Full type checks, ESLint (zero warnings) and Android dev build pass. Production packaging and current-head CI receipts are recorded separately at delivery; earlier d16 CI had two intermittent editor component failures plus Qodana licensing and Codacy complexity gates.

## Merge-gate test synchronization (2026-09-22)
The full 285fc90 CI run passed 693/694 component cases; all MobileLayout and both new refresh regressions passed. The remaining failure read a one-time IndexedDB snapshot immediately after the confirmation dialog disappeared. Radix closes that dialog on click, before remote and local deletion finish, and Cypress retries assertions against the same array instead of rereading storage. A 500ms DELETE response delay reproduces the failure locally with CI coverage enabled. Wait for the reading pane to show `No Note Selected` (after durable deletion) before checking the cache, then retain the zero-cache and remount assertions. This changes test synchronization only; the production APK remains valid.
