...
- **Static analysis note**: Codacy's `ESLint8_xss_no-mixed-html` reports ~18 findings across this module. It infers "this value is HTML" from identifier names, so it fires on every wire-format ↔ column mapping and even on `const char = html[index]`, and it cannot be suppressed per file: `eslint-plugin-xss` is ESLint 8-only and crashes under the project's ESLint 9, so an `eslint-disable` directive for it is itself a lint error here. Editing `.codacy/codacy.config.json` has no effect either — that file is a snapshot of the cloud configuration. Turning the pattern off in the Codacy UI is an open decision for the repository owner. XSS coverage does not depend on it: CodeQL, Semgrep and SonarQube all pass, DOMPurify sanitizes on render and `sanitizeNoteHtml()` on write.
