...
2) Очередь и кеш
   ...
   - [x] Обновить UI-хуки для работы через очередь/кеш (оптимистичные оффлайн-обновления списков/selectedNote).
...
4) UX/поведение
   ...
   - [x] Проверка перезапуска: данные/очередь сохраняются и восстанавливают актуальный вид списков/деталей.
...
- Mobile: отсутствует реализация SQLite/AsyncStorage адаптера — требуется отдельная задача.
