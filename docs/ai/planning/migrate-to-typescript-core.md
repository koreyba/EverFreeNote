...
- [ ] Расширить доменные типы (Tag, RPC/REST ответы, IndexedDB схемы) и применить в хуках/утилитах.
- [ ] Перевести `lib/indexeddb/**` и прочие утилиты/скрипты (`scripts/*.js`) на TS с сохранением функционала (частично готово: supabase скрипты в `.ts`; остаются `db_audit_scripts/*.js`).
...
- [ ] Финализация миграции: отключить `allowJs`, удалить `jsconfig.json`, очистить оставшиеся `.js/.jsx` вне UI (координация с Часть A).
