...
## Текущий статус
- Готово: почти вся UI-библиотека на TSX (все кроме `components/ui/sidebar\.tsx`).
- Доменные типы старт: `types/domain.ts`.
- `tsc --noEmit` проходит на текущем наборе TS-файлов.

## Задачи (не пересекаются с Часть B)
- [x] Перевести `components/ui/sidebar\.tsx` → `.tsx`.
- [x] Перевести фичевые компоненты (`components/*\.tsx`) на TSX, подключить общие типы (`Note`, RPC-ответы).
- [x] Перевести страницы `app/*.js` на TSX, типизировать роуты/props, заменить dynamic import без типов.
- [ ] API-роуты `app/api/**` → TS, типы request/response.
- [x] Обновить Cypress конфиги/тесты под TS (component + e2e) или `// @ts-check`.
...
## Выходные артефакты
- Все клиентские файлы на TS/TSX, отсутствуют `\.tsx`/`.js` в UI.
